...
//DIALOGO DE BORRADO
	$( "#dialogoborrar" ).dialog({
		...
		"Borrar": function() {			
			//Ajax con get
			$.get("inmueble_borrar.php", {"idinmueble":idinmueble},function(){				
				$("#inmueble_" + idinmueble).fadeOut(500,function(){$(this).remove();});
			})//get			
			//Cerrar la ventana de dialogo				
			$(this).dialog("close");												
		},
...
$( "#dialogomodificar" ).dialog({
		...
		"Guardar": function() {			
			$.post("inmueble_modificar.php", {
				...
				textotipo: $("#idtipomodificar :selected").text(),
				idtipo: $("#idtipo").val() ,
				visitamodificar : $("#visitamodificar").val()
			},function(data,status){
				$("#inmueble_" + idinmueble).html(data);
	...
	$(document).on("click",".direccion",function(){
		...
		//var visitas= $(this).parent().siblings("td.direccion").html();
		//console.log(visitas);
	});	
...
$(document).on("click",".modificar",function(){
	...
	idinmueble = $(this).parents("tr").data("idinmueble");
	//Para que ponga el campo direccion con su valor $(this).parent().siblings("td.direccion").html()
	var direc= $(this).parent().siblings("td.direccion").data("direccion");
	$("#direccionmodificar").val(direc);
	
	//Para que me seleccione el idtipomodificar
	var idtipomodificar = $(this).parent().siblings("td.idtipo").data("idtipo");
...
$(document).on("click",".incrementavisita a",function(){	
	//Obtener el idinmueble a través del atributo data-idinmueble
	idinmueble = $(this).parents("tr").data("idinmueble");	
...
$(document).on("click","#guardarnuevo",function(){
	$.post("inmueble_insertar_nuevo.php", {
				...
				textotipo:$("#idtiponuevo :selected").text(),
